...
- กด **End** หรือ grip ค้าง 1 วิ → AI วิเคราะห์ → **แสดง Score Report ใน VR โดยไม่ต้องออก** → กด Done เพื่อออก

### 6. VR Video Mode
- วิดีโอ 180° SBS loop ใน VR environment
- กด controller → Exit button ปรากฏ → เล็ง → กด → ออก VR

### 7. Score Report (Dark Holographic UI)
- Overall score 0–100 พร้อม animated progress ring — แสดงเป็น centered banner
...
- Filler word tags, Key Strengths, To Improve list
...
Enter VR
  ...
  → End / grip 1s
  → ส่ง audio → /api/presentation/transcribe → Groq Whisper (verbose_json)
      → filter segments ที่ no_speech_prob ≥ 0.6 ออก (ป้องกัน hallucination)
  → ถ้า transcript < 20 ตัวอักษร → ใช้ fallback score ทันที
  → ส่ง transcript → /api/presentation/analyze → Groq Llama 3.3
      → sanitize output: strip CJK characters, filter filler key นอก schema
  → แสดง Score Report ใน VR → กด Done เพื่อออก
```

**Slide Image Proxy:**
```
VRScene โหลด slide URL → /api/proxy-image?url=<r2-url>
  → validate host ตรง R2_PUBLIC_DOMAIN
  → fetch จาก R2 server-side → return พร้อม Cache-Control 1 วัน
  (ป้องกัน CORS error จาก pub-*.r2.dev)
```
...
### กรณีมี transcript (AI วิเคราะห์ด้วย Llama) — รวม 100 คะแนน

| หมวด | น้ำหนัก | เกณฑ์ |
|---|---|---|
| Filler Words | /25 | 0คำ=25, 1-2=20, 3-5=15, 6-10=10, >10=5 |
| Fluency | /25 | ต่อเนื่อง=21-25, หยุดนิดหน่อย=15-20, หยุดบ่อย=8-14, ไม่ลื่น=1-7 |
| Structure | /25 | ครบ intro+body+conclusion=21-25, บางส่วน=12-20, ขาด=1-11 |
| Time Management | /25 | 45-90วิ/สไลด์=25, 30-44=20, 91-120=18, 15-29=12, 121-180=10, นอกนั้น=5 |
...
| Structure (Slide Coverage) | 100%=25, 75-99%=18, 50-74%=12, 25-49%=6, <25%=2 |

- ใช้ **Groq Llama 3.3-70b-versatile** (temperature 0.3)
- ใช้ **Groq Whisper `whisper-large-v3-turbo`** (verbose_json — filter no_speech_prob ≥ 0.6)
- Guard: transcript < 20 ตัวอักษร → ใช้ fallback ทันที
- Sanitize: ตัดอักษร CJK ที่ Llama inject มา + filter filler key นอก schema ออก
